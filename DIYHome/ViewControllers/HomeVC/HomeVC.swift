...
let CommonCodeObj = CommonCode();
let ConstantObj = Constant();
let WSCallObj = CommonWSCall();


class HomeVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, userPostdelegate, userProfiledelegate, FaveButtonDelegate, UISearchBarDelegate
{
    //MARK: - Outlets
    ...
    var dictForUserData = [String: String]();
    ...
    func getUserPostData_returnResp(aArrJsonResponse: [Any])
     {
         ...
         if aArrJsonResponse.count > 0
         {
            ...
            //NOTE: Here we remove(filter) current user's all posts. You can see all current user's posts in My Profile tab.
